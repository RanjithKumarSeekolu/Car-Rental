...
const CarFiltersSidebar = ({ filters, setFilters, clearFilters, isOpen, onClose }) => {
  ...
  return (
    <div className={`bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex-shrink-0 custom-scrollbar transition-all
                ...
                {categories.map(cat => (
                    <label key={cat} className="flex items-center space-x-3 cursor-pointer group">
                        <input 
                            ...
                            className="w-5 h-5 border-gray-300 rounded text-indigo-600 focus:ring-indigo-500 transition-colors"
                        />
                ...
                {seats.map(opt => (
                    <label key={opt} className="flex items-center space-x-3 cursor-pointer group">
                        <input 
                            ...
                            className="w-5 h-5 border-gray-300 rounded text-indigo-600 focus:ring-indigo-500 transition-colors"
                        />
                        <span className="text-gray-600 group-hover:text-gray-900 transition-colors">{opt}</span>
                    </label>
                ))}
            </div>
        </div>
