...
const UserMenu = ({ closeModal, logoutHandler }) => {
  ...
  return (
    <div 
      ref={menuRef}
      className="absolute top-0 right-0 w-48 bg-white rounded-lg shadow-xl border border-gray-100 z-50 overflow-hidden py-1 animate-in fade-in zoom-in-95 duration-200"
    >
      ...
      <Link 
          to="/dashboard"
          onClick={closeModal}
          className="block px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 hover:text-indigo-800 transition-colors"
      >
          My Dashboard
      </Link>
      <Link 
